document.addEventListener('DOMContentLoaded', function () {
    ...
    var gemini37FlashApiKeyInput = document.getElementById('gemini37FlashApiKey');
    var gemini38FlashApiKeyInput = document.getElementById('gemini38FlashApiKey');
        ...
        chrome.storage.sync.get('language', function (data) {
            ...
            const translations = {
                ko: {
                    ...
                    apiKeyGoogle30Flash: 'Google 3.0 플래시 API 키:',
                    apiKeyGoogle30FlashPlaceholder: 'Google 3.0 플래시 API 키를 입력하세요',
                    apiKeyGoogle31FlashLite: 'Google 3.1 플래시 라이트 API 키:',
                    ...
                    apiKeyGoogle37Flash: 'Google 3.7 플래시 API 키:',
                    apiKeyGoogle37FlashPlaceholder: 'Google 3.7 플래시 API 키를 입력하세요',
                    apiKeyGoogle38Flash: 'Google 3.8 플래시 API 키:',
                    apiKeyGoogle38FlashPlaceholder: 'Google 3.8 플래시 API 키를 입력하세요',
                },
                en: {
                    ...
                    apiKeyGoogle30Flash: 'Google 3.0 Flash API Key:',
                    apiKeyGoogle30FlashPlaceholder: 'Enter Google 3.0 Flash API Key',
                    apiKeyGoogle31FlashLite: 'Google 3.1 Flash Lite API Key:',
                    ...
                    apiKeyGoogle37Flash: 'Google 3.7 Flash API Key:',
                    apiKeyGoogle37FlashPlaceholder: 'Enter Google 3.7 Flash API Key',
                    apiKeyGoogle38Flash: 'Google 3.8 Flash API Key:',
                    apiKeyGoogle38FlashPlaceholder: 'Enter Google 3.8 Flash API Key',
                }
            };
            ...
            apiInputs.forEach(input => {
                ...
                if (input.id !== 'google20FlashApiKey' && input.id !== 'gemini25FlashApiKey' && input.id !== 'gemini3FlashApiKey' && input.id !== 'gemini31FlashLiteApiKey' && input.id !== 'gemini35FlashApiKey' && input.id !== 'gemini36FlashApiKey' && input.id !== 'gemini35FlashLiteApiKey' && input.id !== 'gemini37FlashApiKey' && input.id !== 'gemini38FlashApiKey' && input.id !== 'ollamaModelName' && input.id !== 'instructionInput') {
            ...
            // Specific translations for Gemini 3.0 Flash
            const gemini3FlashLabel = document.querySelector('label[for="gemini3FlashApiKey"]');
            if (gemini3FlashLabel) {
                gemini3FlashLabel.textContent = translations[lang].apiKeyGoogle30Flash || 'Google 3.0 Flash API Key:';
            }
            if (gemini3FlashApiKeyInput) {
                gemini3FlashApiKeyInput.placeholder = translations[lang].apiKeyGoogle30FlashPlaceholder || 'Enter Google 3.0 Flash API Key';
            }

            // Specific translations for Gemini 3.1 Flash Lite
            const gemini31FlashLiteLabel = document.querySelector('label[for="gemini31FlashLiteApiKey"]');
            if (gemini31FlashLiteLabel) {
                gemini31FlashLiteLabel.textContent = translations[lang].apiKeyGoogle31FlashLite || 'Google 3.1 Flash Lite API Key:';
            }
            if (gemini31FlashLiteApiKeyInput) {
                gemini31FlashLiteApiKeyInput.placeholder = translations[lang].apiKeyGoogle31FlashLitePlaceholder || 'Enter Google 3.1 Flash Lite API Key';
            }

            // Specific translations for Gemini 3.5 Flash
            const gemini35FlashLabel = document.querySelector('label[for="gemini35FlashApiKey"]');
            if (gemini35FlashLabel) {
                gemini35FlashLabel.textContent = translations[lang].apiKeyGoogle35Flash || 'Google 3.5 Flash API Key:';
            }
            ...
            // Specific translations for Gemini 3.7 Flash
            const gemini37FlashLabel = document.querySelector('label[for="gemini37FlashApiKey"]');
            if (gemini37FlashLabel) {
                gemini37FlashLabel.textContent = translations[lang].apiKeyGoogle37Flash || 'Google 3.7 Flash API Key:';
            }
            if (gemini37FlashApiKeyInput) {
                gemini37FlashApiKeyInput.placeholder = translations[lang].apiKeyGoogle37FlashPlaceholder || 'Enter Google 3.7 Flash API Key';
            }

            // Specific translations for Gemini 3.8 Flash
            const gemini38FlashLabel = document.querySelector('label[for="gemini38FlashApiKey"]');
            if (gemini38FlashLabel) {
                gemini38FlashLabel.textContent = translations[lang].apiKeyGoogle38Flash || 'Google 3.8 Flash API Key:';
            }
            if (gemini38FlashApiKeyInput) {
                gemini38FlashApiKeyInput.placeholder = translations[lang].apiKeyGoogle38FlashPlaceholder || 'Enter Google 3.8 Flash API Key';
            }
        });
    }
    ...
    function updateUI(data) {
        ...
        // gemini3FlashApiKey 설정
        if (gemini3FlashApiKeyInput) {
            gemini3FlashApiKeyInput.value = data.gemini3FlashApiKey || '';
        }

        // gemini31FlashLiteApiKey 설정
        if (gemini31FlashLiteApiKeyInput) {
            gemini31FlashLiteApiKeyInput.value = data.gemini31FlashLiteApiKey || '';
        }

        // gemini35FlashApiKey 설정
        if (gemini35FlashApiKeyInput) {
        ...
        // gemini37FlashApiKey 설정
        if (gemini37FlashApiKeyInput) {
            gemini37FlashApiKeyInput.value = data.gemini37FlashApiKey || '';
        }

        // gemini38FlashApiKey 설정
        if (gemini38FlashApiKeyInput) {
            gemini38FlashApiKeyInput.value = data.gemini38FlashApiKey || '';
        }
    ...
    function updateApiKeyVisibility(selectedModel) {
        ...
        const gemini37FlashSection = document.getElementById('gemini37FlashApiSection');
        const gemini38FlashSection = document.getElementById('gemini38FlashApiSection');
        ...
        if (gemini37FlashSection) gemini37FlashSection.style.display = 'none';
        if (gemini38FlashSection) gemini38FlashSection.style.display = 'none';
        ...
        switch (selectedModel) {
            ...
            case 'gemini25Flash':
                ...
                break;
            case 'gemini3Flash':
                if (gemini3FlashSection) {
                    gemini3FlashSection.style.display = 'block';
                }
                break;
            case 'gemini31FlashLite':
                if (gemini31FlashLiteSection) {
                    gemini31FlashLiteSection.style.display = 'block';
                }
                break;
            case 'gemini35Flash':
                if (gemini35FlashSection) {
            ...
            case 'gemini35FlashLite':
                ...
                break;
            case 'gemini37Flash':
                if (gemini37FlashSection) {
                    gemini37FlashSection.style.display = 'block';
                }
                break;
            case 'gemini38Flash':
                if (gemini38FlashSection) {
                    gemini38FlashSection.style.display = 'block';
                }
                break;
            case 'groq':
    ...
    saveButton.addEventListener('click', function () {
        ...
        var gemini37FlashApiKey = gemini37FlashApiKeyInput ? gemini37FlashApiKeyInput.value.trim() : '';
        var gemini38FlashApiKey = gemini38FlashApiKeyInput ? gemini38FlashApiKeyInput.value.trim() : '';
        ...
        var dataToSave = {
            ...
            gemini37FlashApiKey: gemini37FlashApiKey,
            gemini38FlashApiKey: gemini38FlashApiKey,
